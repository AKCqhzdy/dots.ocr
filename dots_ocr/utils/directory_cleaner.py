...
class SectionHeader:
    def __init__(self, text, category:str, bbox, level=None):
        self.text = text
...
class DirectoryStructure:
    ...
    def add_header(self, text, category, bbox):
        header = SectionHeader(text, category, bbox)
    ...
    def load_from_json(self, json_data):
        for info_block in json_data:
            if info_block.get('category') == 'Section-header' or info_block.get('category') == 'Title' or info_block.get('category') == 'List-item':
                self.add_header(info_block['text'], info_block['category'], info_block['bbox'])
...
class Reranker():
    ...
    def identify_highest_headers(self, cells, y_offset_list):
        ...
        for info_block in cells["full_layout_info"]:
            # print(info_block)
            now_header = SectionHeader(info_block['text'], info_block['category'], info_block['bbox'])

            match_header_idx = []
            while h_idx < len(y_offset_list) and y_offset_list[h_idx] < info_block["bbox"][3] + 5:
        ...
        new_check_list = []
        SAVE_NUM = 10 
        for i in range(min(SAVE_NUM, len(highest_headers_idx))):
    ...
    def insert(self, header: SectionHeader):
        ...
        if header.level < self.now_level and header.level >= self.upper_level:
    ...
    async def merge_crops_and_parse(self, crops, save_dir=None, save_name=None):
        ...
        merged_image.save(f"test/output/output{self.count}.jpg")
        self.count += 1
        ...
        while self.reranker.upper_level <= LEVEL_NUM:
            ...
            for i, sorted_indices in enumerate(all_sorted_indices):
                ...
                while j < len(sorted_indices):
                    ...
                    if headers[sorted_indices[j]].level <= max_level_in_this_page + 1:
